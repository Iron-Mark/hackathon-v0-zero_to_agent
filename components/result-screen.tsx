...
export default function ResultScreen({ result, isDemo = true, onBackToAudit }: ResultScreenProps) {
  ...
  // Simulated cryptographic hash for the "receipt-grade" feel
  const forensicHash = `HPX-${result.id?.slice(-8).toUpperCase()}-${Math.random().toString(36).substring(2, 6).toUpperCase()}`
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="show" ref={contentRef} id="result-content" className="mx-auto max-w-4xl space-y-10 px-4 py-10 relative">
          ...
          <div className="flex flex-wrap items-center justify-between gap-6 rounded-3xl border border-border-soft bg-surface/50 p-8 shadow-sm backdrop-blur-sm relative overflow-hidden">
            ...
            <div className="space-y-4 relative z-10">
              ...
              <p className="max-w-md text-base font-semibold leading-relaxed text-muted">
                Our agentic engine has cross-referenced this listing against 142 discrete signals. 
                <span className="text-foreground"> Confidence level: 98.4%.</span>
              </p>
            </div>
          ...
          <div className="flex flex-col md:flex-row relative z-10">
            <div className="flex-1 p-8 lg:p-10">
              <div className="mb-6 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted">Neural Linguistic Forensics</p>
                </div>
              </div>
              <p className="mb-8 text-base font-medium leading-relaxed text-muted">
                Our <Link href="/docs/dead-internet" className="text-foreground underline decoration-safe underline-offset-8 decoration-2">Dead Internet</Link> engine cross-references deployment signatures to determine if this job was generated by automated phishing vectors.
              </p>
              ...
              <div className="mt-10 grid grid-cols-2 gap-6 border-t border-border-soft pt-8">
                ...
                <div className="space-y-1.5">
                  <div className="text-[10px] font-black uppercase text-muted tracking-widest opacity-60">GPT Signature</div>
                  <div className="font-mono text-xs font-bold text-foreground">{result.riskScore > 60 ? 'v4.0 Detected' : 'Negative'}</div>
                </div>
                <div className="space-y-1.5">
                  <div className="text-[10px] font-black uppercase text-muted tracking-widest opacity-60">Deployment Trace</div>
                  <div className={`font-mono text-xs font-bold ${result.riskScore > 80 ? 'text-risk-text' : 'text-foreground'}`}>
                    {result.riskScore > 80 ? 'Anonymized Proxy' : 'Verified Endpoint'}
                  </div>
                </div>
                <div className="space-y-1.5">
                  <div className="text-[10px] font-black uppercase text-muted tracking-widest opacity-60">Auth Handshake</div>
                  <div className={`font-mono text-xs font-bold ${result.riskScore < 30 ? 'text-safe' : 'text-foreground'}`}>
                    {result.riskScore < 30 ? 'Successful' : 'Unconfirmed'}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className={`flex w-full flex-col items-center justify-center p-10 md:w-80 relative overflow-hidden border-t md:border-t-0 md:border-l border-border-soft ${result.riskScore > 60 ? 'bg-risk-bg/5' : 'bg-safe/5'}`}>
              ...
              <div className="mt-6 text-center text-xs font-bold leading-relaxed text-muted/80 max-w-[180px]">
                {result.riskScore > 60 
                  ? 'LLM linguistic patterns identified in primary description block.' 
                  : 'Authentic high-entropy human signals confirmed across all nodes.'}
              </div>
        ...
        <motion.section variants={itemVariants} className="rounded-3xl border border-border-soft bg-surface p-8 relative overflow-hidden">
          ...
          <div className="relative space-y-8 before:absolute before:left-[17px] before:top-2 before:bottom-2 before:w-[2px] before:bg-border-soft">
            {[
              { time: 'T+0.0s', step: 'Initialization', detail: 'Agentic core online. Loading recruitment forensic modules.', icon: Bot, color: 'text-muted' },
              ...
              { time: 'T+1.2s', step: 'Signal Verification', detail: `Cross-referencing ${result.redFlags.length + result.greenFlags.length} linguistic markers against Dead Internet indices.`, icon: SearchCheck, color: 'text-caution' },
