  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-7xl px-4 py-12 lg:px-8">
        ...
        <section className="mb-8 rounded-3xl border border-border-soft bg-surface p-6 shadow-sm">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div className="max-w-3xl">
              <div className="mb-3 flex items-center gap-3">
                <Terminal className="h-5 w-5 text-evidence" />
                <h2 className="text-xl font-black">Local Research Agents</h2>
              </div>
              <p className="text-sm font-semibold leading-6 text-muted">
                Cursor is the primary local research runner for suspicious job posts and recruiter messages. Codex SDK is available as an opt-in fallback when Cursor is unavailable or fails, and saved reports stay local under ignored artifacts.
              </p>
            </div>
            <Link
              href="/docs"
              className="hireproof-focus inline-flex items-center justify-center gap-2 rounded-xl border border-border-soft bg-background px-4 py-3 text-xs font-black uppercase tracking-widest text-muted transition-colors hover:border-evidence/50 hover:text-foreground"
            >
              Research Docs <ArrowUpRight className="h-4 w-4" />
            </Link>
          </div>
          <div className="mt-5 grid gap-3 lg:grid-cols-2">
            {[
              'npm run research:agent -- --prompt "Check this recruiter message"',
              'npm run research:agent -- --file ./job-post.txt --enable-codex --save',
            ].map((command) => (
              <button
                key={command}
                onClick={() => copy(command)}
                className="hireproof-focus flex min-h-14 items-center justify-between gap-3 rounded-2xl border border-border-soft bg-background px-4 py-3 text-left font-mono text-xs font-bold text-foreground transition-colors hover:border-safe/40"
              >
                <span className="break-all">{command}</span>
                <Copy className="h-4 w-4 shrink-0 text-safe" />
              </button>
            ))}
          </div>
        </section>
