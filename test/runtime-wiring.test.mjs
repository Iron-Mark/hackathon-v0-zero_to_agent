...
test('pilot intake stores requests and exposes authenticated admin/export surfaces', async () => {
  const store = await fs.readFile(new URL('../lib/auth-store.ts', import.meta.url), 'utf8')
  const intake = await fs.readFile(new URL('../app/pilot/pilot-intake-client.tsx', import.meta.url), 'utf8')
  ...
  assert.match(admin, /updateStatus/)
})

test('lightweight product analytics record demo, pilot, and case-study events', async () => {
  const store = await fs.readFile(new URL('../lib/auth-store.ts', import.meta.url), 'utf8')
  const analyticsRoute = await fs.readFile(new URL('../app/api/analytics/events/route.ts', import.meta.url), 'utf8')
  const developerAnalytics = await fs.readFile(new URL('../app/api/developer/analytics/route.ts', import.meta.url), 'utf8')
  const analyticsExport = await fs.readFile(new URL('../app/api/developer/analytics/export/route.ts', import.meta.url), 'utf8')
  const tracker = await fs.readFile(new URL('../components/analytics/product-event-tracker.tsx', import.meta.url), 'utf8')
  const portfolio = await fs.readFile(new URL('../app/portfolio/page.tsx', import.meta.url), 'utf8')
  const pilot = await fs.readFile(new URL('../app/pilot/page.tsx', import.meta.url), 'utf8')
  const audit = await fs.readFile(new URL('../app/audit/audit-client.tsx', import.meta.url), 'utf8')
